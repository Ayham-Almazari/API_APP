...
trait Auth
{
    ...
    foreach ($identifiers as list($identifier,$password)) :
        ...
        $token = $this->authenticate_user($model,$user,$credentials,$request);//authenticate
        //if authenticated
    ...
    public function get_data($kies=null,$values=null) {
        $user=collect(auth()->user())->all();
        $profile=collect(auth()->user()->profile)->except('admin_id','owner_id','buyer_id');
        return [
            "user"=>$user,
            "profile"=>$profile,
